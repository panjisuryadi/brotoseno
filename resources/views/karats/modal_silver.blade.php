<div class="px-3">
      <x-library.alert />
      <form id="FormEdit" action="{{ route('silver.update', $detail) }}" method="POST">
          @csrf
          @method('patch')
          <div class="flex flex-row grid grid-cols-1 gap-4">
              ...
              <div class="form-group">
                  ...
                  <input class="form-control" type="text" name="{{ $field_name }}" step="0.01" id="{{ $field_name }}" value="{{$detail->kode }}">
              </div>
              ...
              <div class="form-group">
                  <?php
                    ...
                    ?>
                  <label for="{{ $field_name }}">{{ $field_lable }}</label>
                  <input class="form-control" type="text" name="{{ $field_name }}" step="0.01" id="{{ $field_name }}" value="{{$detail->name }}">
              </div>
              ...
              <div class="form-group">
                  <?php
                    ...
                    $invalid = $errors->has($field_name) ? ' is-invalid' : '';
                    $required = "required";
                    $harga  = (int)str_replace(',', '', $detail->harga, );
                    ?>
                  <label for="{{ $field_name }}">{{ $field_lable }}<span class="text-danger">*</span></label>
                  <input type="hidden" name="id" value="{{$detail->id}}">
                  <input class="form-control" type="number" name="{{ $field_name }}" step="0.01" id="{{ $field_name }}" value="{{$harga }}">
              </div>
          </div>
      </form>
  </div>
