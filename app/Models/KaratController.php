...
class KaratController extends Controller
{
    ...
    public function list_lm(Request $request)
    {
        // $harga = Harga::where('tanggal', date('Y-m-d'))->first();
        $harga = Harga::latest()->first();
        // echo json_encode($harga);
        ...
        $dataKarat = Karat::whereNull('parent_id')->get();
        return view(
            'karats.list_lm', // Path to your create view file
            compact(
                'module_title',
                'module_name',
                'module_path',
    ...
    public function index_lm(Request $request)
    {
        $module_title = $this->module_title;
        $module_name = $this->module_name;
        $module_path = $this->module_path;
        ...
        $karat = Karat::where('type', 'LM')->latest()->get();
    ...
    public function update_lm(Request $request)
    {
        ...
        $data->harga = $harga;
        $data->save();
        return response()->json(['success'=>'LM Sukses diupdate.']);
        // return redirect()->action([KaratController::class, 'list_lm']);
    }
    ...
    public function index_diskon(Request $request)
    {
        ...
        // $harga  = 1000000;
        $$module_name = Karat::where('status', 'A')->where('type', 'GOLD')->latest()->get();
