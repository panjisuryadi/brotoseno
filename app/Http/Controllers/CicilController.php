...
class CicilController extends Controller
{
    ...
    public function list() {
        ...
        return view(
            'cicil.list', // Path to your create view file
            compact(
                'module_title',
                'module_name',
                'module_path',
    ...
    public function list_data(Request $request) {
        ...
        $cicil  = Cicil::with('customers', 'sales_gold')->where('id', $id)->first();

        return view(
            'cicil.list_data', // Path to your create view file
            compact(
                'id',
                'cicil',
                'module_title',
    ...
    public function index_data()
    {
        ...
        $module_action = 'List';
        $$module_name = Cicil::with('sales_gold', 'customers')->latest()->get();
        $data = $$module_name;
        return Datatables::of($data)
            ->addColumn('action', function ($data) {
                ...
                $module_model = $this->module_model;
                $module_path = $this->module_path;
                return view('cicil.action',
                    compact('module_name', 'data', 'module_model'));
            })

            ->editColumn('tanggal', function($data){
                return '<div class="items-center text-center">' .tgljam($data->created_at) . '</div>';
            }) 

            ->editColumn('trx', function($data){
                return '<div class="items-center text-center">' .($data->sales_gold->nomor) . '</div>';
            }) 

            ->editColumn('customer', function($data){
                return '<div class="items-center text-center">' .($data->customers->customer_name) . '</div>';
            }) 

            ->editColumn('total_trx', function($data){
                return number_format($data->total_trx);
            }) 

            ->editColumn('paid', function($data){
                return number_format($data->paid);
            }) 
            ...
            ->editColumn('status', function($data){
                if($data->sisa == 0){
                    return '<div class="items-center text-center text-success">Lunas</div>';
                }else{
                    return '<div class="items-center text-center text-danger">Cicil</div>';
                }
            })
            
            ->rawColumns(['tanggal', 'harga', 'trx','customer', 'status'])
            ->make(true);
    }
    ...
    public function index_datas(Request $request)
    {
        ...
        $module_action = 'List';
        $$module_name = CicilData::with('sales_gold', 'customers')->latest()->get();
        $data = $$module_name;
        return Datatables::of($data)
            ->addColumn('action', function ($data) {
                $module_name = $this->module_name;
